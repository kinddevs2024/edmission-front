...
export function UniversityDashboard() {
  ...
  const nextAction = !dashboard?.verified
    ? {
        ...
        description: t('university:dashboardVerifyHint', 'Verified universities look more reliable to students and can move faster through the funnel.'),
        to: '/university/profile',
      }
    : interestedCount === 0
      ? {
          title: t('university:dashboardDiscoveryTitle', 'Add discovery momentum'),
          description: t('university:dashboardDiscoveryHint', 'Review recommended students and refine your profile so more students can find your institution.'),
          to: '/university/students',
        }
      : {
          title: t('university:dashboardPipelineTitle', 'Work the warmest leads first'),
          description: t('university:dashboardPipelineHint', 'Reply quickly to interested students and move them into chat or offer stages while they are active.'),
      ...
      <section className="grid gap-4 rounded-card border border-[var(--color-border)] bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(224,242,254,0.82))] p-5 shadow-[var(--shadow-card)] dark:bg-[linear-gradient(135deg,rgba(17,24,39,0.98),rgba(8,47,73,0.94))] lg:grid-cols-[minmax(0,1.6fr)_minmax(18rem,0.95fr)] lg:items-start sm:p-7">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={dashboard?.verified ? 'success' : 'default'} className="inline-flex items-center gap-1">
              {dashboard?.verified ? <ShieldCheck size={14} /> : <Sparkles size={14} />}
              {dashboard?.verified ? t('university:verifiedStatus', 'Verified and visible') : t('university:verificationPending', 'Verification in progress')}
            </Badge>
            <Badge variant="default">
              {t('university:newInterests', 'New interests')}: {loading ? '—' : interestedCount}
            </Badge>
          </div>
          <div>
            <h2 className="text-2xl font-semibold text-[var(--color-text)]">
              {t('university:dashboardHeroTitle', 'Keep recruitment clear, fast, and easy to act on')}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--color-text-muted)]">
              {t('university:dashboardHeroHint', 'This space is organized around the actions that matter most for university teams: discover students, answer faster, and convert interest into offers.')}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button to="/university/chat" variant="ghost">{t('university:activeChats', 'Active chats')}</Button>
          </div>
        </div>
        ...
        <div className="rounded-[26px] border border-white/60 bg-[var(--color-card)]/88 p-5 shadow-[0_24px_60px_-36px_rgba(15,23,42,0.45)] backdrop-blur">
          ...
          <p className="mt-2 text-lg font-semibold text-[var(--color-text)]">{nextAction.title}</p>
          <p className="mt-2 text-sm leading-relaxed text-[var(--color-text-muted)]">{nextAction.description}</p>
          <Link
            to={nextAction.to}
