...
export function OfferCertificateView({ offer }: OfferCertificateViewProps) {
  ...
  const title = (offer as any).certificateTitle ?? t('offerCertificate.defaultTitle', 'Offer certificate')
      ...
      <div className="relative space-y-4">
        <div className="text-center">
          <p className="text-xs uppercase tracking-[0.25em] text-[var(--color-text-muted)]">
            {t('offerCertificate.certificateLabel', 'Certificate of Offer')}
          </p>
        ...
        <div className="space-y-2 text-sm text-[var(--color-text-muted)] text-center">
          {studentName && (
            <p className="text-[var(--color-text)] font-medium">
              {t('offerCertificate.dear', { name: studentName, defaultValue: 'Dear {{name}},' })}
            </p>
          )}
          {body ? body.split('\n').map((line: string, idx: number) => (
            ...
            <p>
              {t('offerCertificate.defaultBody', {
                university: offer.universityName ?? offer.universityId,
                defaultValue: 'You have received an academic offer from {{university}}.',
              })}
            </p>
          )}
        </div>
