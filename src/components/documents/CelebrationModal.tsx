    ...
    <Modal
      ...
      title={(
        <div className="flex items-center gap-2">
          ...
          <span>You received {type === 'offer' ? 'an Offer' : 'a Scholarship'}</span>
        </div>
      )}
      footer={<Button onClick={onView}>View document</Button>}
    >
      <div className="relative overflow-hidden rounded-[28px] border border-[var(--color-border)] bg-[radial-gradient(circle_at_top,#fef3c7,transparent_45%),linear-gradient(135deg,#eff6ff_0%,#f8fafc_55%,#ecfeff_100%)] p-8 text-center">
        ...
        <div className="relative space-y-3">
          <p className="text-xs uppercase tracking-[0.3em] text-[var(--color-text-muted)]">Congratulations</p>
          ...
          <p className="mx-auto max-w-xl text-sm text-[var(--color-text-muted)]">
            Your personalized {DOCUMENT_TYPE_LABELS[type].toLowerCase()} is ready. Open it to review the full details and make a decision.
          </p>
        </div>
      </div>
    </Modal>
